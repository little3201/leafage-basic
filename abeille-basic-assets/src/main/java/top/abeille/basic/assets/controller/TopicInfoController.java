...
public class TopicInfoController extends AbstractController {
    ...
    /**
     * 根据传入的业务id: topicId 查询信息
     *
     * @param topicId 业务id
     * @return 如果查询到数据，返回查询到的信息，否则返回404状态码
     */
    @GetMapping("/{topicId}")
    public Mono<ResponseEntity<TopicVO>> fetchTopic(@PathVariable String topicId) {
        return topicInfoService.fetchById(topicId)
    ...
    /**
     * 根据传入的业务id: topicId 和要修改的数据，修改信息
     *
     * @param topicId  业务id
     ...
     */
    @PutMapping("/{topicId}")
    public Mono<ResponseEntity<TopicVO>> modifyTopic(@PathVariable String topicId, @RequestBody @Valid TopicDTO topicDTO) {
        return topicInfoService.modify(topicId, topicDTO)
                .map(ResponseEntity::ok)
