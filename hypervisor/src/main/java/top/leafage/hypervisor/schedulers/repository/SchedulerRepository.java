...
package top.leafage.hypervisor.schedulers.repository;
...
/**
 * Repository interface for schedulers.
...
public interface SchedulerRepository extends JpaRepository<Scheduler, Long>, JpaSpecificationExecutor<Scheduler> {
     ...
     */
    @Modifying
    @Query("UPDATE Scheduler t SET t.enabled = true WHERE t.id = :id AND t.enabled = false")
     ...
     */
    @Modifying
    @Query("UPDATE Scheduler t SET t.enabled = false WHERE t.id = :id AND t.enabled = true")
