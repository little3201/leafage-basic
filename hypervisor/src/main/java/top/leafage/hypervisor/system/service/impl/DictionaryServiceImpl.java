...
public class DictionaryServiceImpl implements DictionaryService {
     ...
     */
    public DictionaryServiceImpl(DictionaryRepository dictionaryRepository, JdbcAggregateTemplate jdbcAggregateTemplate) {
    ...
    public Page<@NonNull DictionaryVO> retrieve(int page, int size, String sortBy, boolean descending, String filters) {
        ...
        List<DictionaryVO> voList = jdbcAggregateTemplate.findAll(query(criteria).with(pageable), Dictionary.class)
                .stream().map(entity -> {
                    long count = dictionaryRepository.countBySuperiorId(entity.getId());
                    return DictionaryVO.from(entity, count);
                })
                .toList();
        return new PageImpl<>(voList, pageable, jdbcAggregateTemplate.count(query(criteria), Dictionary.class));
    }
    ...
    public DictionaryVO fetch(Long id) {
        ...
        return dictionaryRepository.findById(id)
                ...
                .orElseThrow(() -> new NoSuchElementException("dictionary not found: " + id));
    }
    ...
    public boolean enable(Long id) {
        ...
        if (!dictionaryRepository.existsById(id)) {
            throw new NoSuchElementException("dictionary not found: " + id);
        }
