...
public class PrivilegeServiceImpl implements PrivilegeService {
    ...
    public PrivilegeServiceImpl(RoleMembersRepository roleMembersRepository, RolePrivilegesRepository rolePrivilegesRepository,
                                PrivilegeRepository privilegeRepository, GroupMembersRepository groupMembersRepository, GroupRolesRepository groupRolesRepository, GroupPrivilegesRepository groupPrivilegesRepository, JdbcAggregateTemplate jdbcAggregateTemplate) {
    ...
    public Page<@NonNull PrivilegeVO> retrieve(int page, int size, String sortBy, boolean descending, String filters) {
        ...
        List<PrivilegeVO> voList = jdbcAggregateTemplate.findAll(query(criteria).with(pageable), Privilege.class)
                .stream().map(entity -> {
                    long count = privilegeRepository.countBySuperiorId(entity.getId());
                    return PrivilegeVO.from(entity, count);
                })
                .toList();
        return new PageImpl<>(voList, pageable, jdbcAggregateTemplate.count(query(criteria), Privilege.class));
    }
    ...
    public PrivilegeVO fetch(Long id) {
        ...
        return privilegeRepository.findById(id)
                ...
                .orElseThrow(() -> new NoSuchElementException("privilege log not found: " + id));
    }
    ...
    public boolean enable(Long id) {
        ...
        if (!privilegeRepository.existsById(id)) {
            throw new NoSuchElementException("privilege not found: " + id);
        }
