...
public class RoleServiceImpl implements RoleService {
     ...
     */
    public RoleServiceImpl(RoleRepository roleRepository, JdbcAggregateTemplate jdbcAggregateTemplate) {
    ...
    public Page<@NonNull RoleVO> retrieve(int page, int size, String sortBy, boolean descending, String filters) {
        ...
        Criteria criteria = buildCriteria(filters, Role.class);

        List<RoleVO> voList = jdbcAggregateTemplate.findAll(query(criteria).with(pageable), Role.class)
                .stream().map(RoleVO::from)
                .toList();
        return new PageImpl<>(voList, pageable, jdbcAggregateTemplate.count(query(criteria), Role.class));
    }
